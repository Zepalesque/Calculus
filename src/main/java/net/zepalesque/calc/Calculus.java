...
import net.zepalesque.calc.function.ParenthesisHeldFunction;
...
// TODO: More logarithms, system for 'real constants' that can be represented by an integer over an integer,
...
//  also implement integration by parts, inverse trig, etc
...
//        System.out.println("Note that (sec(x) ^ 2) = (tan(x) ^ 2) + 1, so thanks to the arbitrary constant of integration, C, these are equivalent.");
    ...
    public static void printFuncAndDerivative(Func f, String id) {
        System.out.println();
        System.out.printf("%s(x) = %s\n", id, noParenthesisString(f));
        System.out.println("deriv:");
        System.out.printf("%s'(x) = %s\n", id, noParenthesisString(f.derivative()));
        System.out.println();
    }
    public static void printFuncAndAntiderivative(Func f, String id) {
        ...
        System.out.printf("%s(x) = %s\n", ig, i);
        System.out.printf("%s(x) = %s + C\n", ig, noParenthesisString(i.integrate()));
        System.out.println();
    }
    ...
    public static void printDifferentiateAndIntegrate(Func f, String id) {
        ...
        System.out.printf("%s(x) = %s\n", id, i1);
        System.out.printf("%s(x) = %s + C\n", id, noParenthesisString(i1.integrate()));
        System.out.println();
    }
    
    public static String noParenthesisString(Func f) {
        return f instanceof ParenthesisHeldFunction phf ? phf.internalString() : f.toString();
    }
}
